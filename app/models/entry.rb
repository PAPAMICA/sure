class Entry < ApplicationRecord
  ...
  after_create_commit :enqueue_notification_rule_transaction_job, if: :transaction_entry_for_notification_rules?
  ...
  # Bank import often puts "PAYPAL …" in notes or the counterparty name; used for PayPal Transaction Search enrichment.
  def paypal_enrich_candidate?
    transaction? && (notes.to_s.match?(/payp/i) || name.to_s.match?(/payp/i))
  end
  ...
  private
    ...
    def transaction_entry_for_notification_rules?
      entryable_type == "Transaction" && !excluded?
    end

    def enqueue_notification_rule_transaction_job
      NotificationRules::ProcessTransactionEntryJob.perform_later(id)
    end
end
